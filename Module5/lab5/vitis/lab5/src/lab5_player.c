...
void print_menu()
{
    print("Lab 5 DDC Player by Dirk Holzman\n\n\r");
...
void calcPhaseSig (int32_t freq)
{
	...
	XGpio_WriteReg(XPAR_AXI_PHASE_SIG_BASEADDR, XGPIO_DATA_OFFSET, pInc);
...
void calcPhaseTune (int32_t freq)
{
	...
	calc = (double) (freq * pow(2,N)) / fs;
	int32_t pInc = (int32_t) calc;
	XGpio_WriteReg(XPAR_AXI_PHASE_TUNE_BASEADDR, XGPIO_DATA_OFFSET, pInc);
...
int main()
{
    ...
    int32_t freqSig = 30001000; // frequency in Hz
    calcPhaseSig(freqSig);
    XGpio_WriteReg(XPAR_AXI_PHASE_SIG_BASEADDR, XGPIO_DATA2_OFFSET, 1); // Set DDS valid signal high
    int32_t freqTune = 30000000; // frequency in Hz
    calcPhaseTune(freqTune);
    XGpio_WriteReg(XPAR_AXI_PHASE_TUNE_BASEADDR, XGPIO_DATA2_OFFSET, 1); // Set DDS valid signal high
    ...
    while(1)
    {
    	...
    	else if ((keystroke == 'r') || (keystroke == 'R')) // pulse reset line to DAC to set phase (not freq) to 0
    	{
    	    ...
    	    XGpio_WriteReg(XPAR_AXI_PHASE_SIG_BASEADDR, XGPIO_DATA2_OFFSET, 0); // Set DDS valid signal low
    	    XGpio_WriteReg(XPAR_AXI_PHASE_TUNE_BASEADDR, XGPIO_DATA2_OFFSET, 0); // Set DDS valid signal low
